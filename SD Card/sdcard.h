...
//#define USE_SD_DMA_MODE
#define USE_SD_POLLING_MODE
//#define USE_SD_INTERRUPT_MODE
...
#define SDIO_SD_APP_SECURE_READ_MULTIPLE_BLOCK \
    ((uint8_t)18) /* For SD Card only */
#define SDIO_SD_APP_SECURE_WRITE_MULTIPLE_BLOCK \
    ((uint8_t)25)                                    /* For SD Card only */
...
SD_Error SD_ReadMultiBlocks(uint32_t addr,
    uint32_t* readbuff,
    uint16_t BlockSize,
...
SD_Error SD_WriteMultiBlocks(uint32_t addr,
    uint32_t* writebuff,
    uint16_t BlockSize,
    uint32_t NumberOfBlocks);

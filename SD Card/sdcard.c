...
static SD_Error SD_SetDeviceMode(uint32_t Mode);
...
* Return         : SD_Error: SD Card Error code.
*******************************************************************************/
SD_Error InitSdioFats(SD_CardInfo* info)
{
    ...
    if(Status == SD_OK) {
        /*----------------- Select Card --------------------------------*/
        Status = SD_SelectDeselect((u32)(info->RCA << 16));
    }
    ...
    if(Status == SD_OK) {
#ifdef USE_SD_DMA_MODE
        Status = SD_SetDeviceMode(SD_DMA_MODE);
#endif
#ifdef USE_SD_POLLING_MODE
        Status = SD_SetDeviceMode(SD_POLLING_MODE);
#endif
#ifdef USE_SD_INTERRUPT_MODE
...
SD_Error SD_ReadBlock(u32 addr, u32* readbuff, u16 BlockSize)
{
    ...
    if(DeviceMode == SD_POLLING_MODE) {
        ...
        SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND |
                          SDIO_IT_RXOVERR | SDIO_IT_RXFIFOHF | SDIO_IT_STBITERR,
            ENABLE);
...
* Return         : SD_Error: SD Card Error code.
*******************************************************************************/
SD_Error SD_ReadMultiBlocks(u32 addr,
    u32* readbuff,
    u16 BlockSize,
    u32 NumberOfBlocks)
{
    ...
    if(NumberOfBlocks > 1) {
        ...
        if(DeviceMode == SD_POLLING_MODE) {
            ...
            while(!(SDIO->STA & (SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL |
                                    SDIO_FLAG_DATAEND | SDIO_FLAG_DTIMEOUT |
                                    SDIO_FLAG_STBITERR))) {
                if(SDIO_GetFlagStatus(SDIO_FLAG_RXFIFOHF) != RESET) {
            ...
            SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT |
                              SDIO_IT_DATAEND | SDIO_IT_RXOVERR |
...
SD_Error SD_WriteBlock(u32 addr, u32* writebuff, u16 BlockSize)
{
    ...
    if(DeviceMode == SD_POLLING_MODE) {
        ...
        SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND |
                          SDIO_FLAG_TXFIFOHE | SDIO_IT_TXUNDERR |
                          SDIO_IT_STBITERR,
            ENABLE);
        while((TransferEnd == 0) && (TransferError == SD_OK)) {
        }
...
* Return         : SD_Error: SD Card Error code.
*******************************************************************************/
SD_Error SD_WriteMultiBlocks(u32 addr,
    u32* writebuff,
    u16 BlockSize,
    u32 NumberOfBlocks)
{
    ...
    if(NumberOfBlocks > 1) {
        ...
        if(DeviceMode == SD_POLLING_MODE) {
            while(!(SDIO->STA & (SDIO_FLAG_TXUNDERR | SDIO_FLAG_DCRCFAIL |
                                    SDIO_FLAG_DATAEND | SDIO_FLAG_DTIMEOUT |
                                    SDIO_FLAG_STBITERR))) {
                if(SDIO_GetFlagStatus(SDIO_FLAG_TXFIFOHE) != RESET) {
            ...
            SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT |
                              SDIO_IT_DATAEND | SDIO_IT_TXUNDERR |
                              SDIO_IT_STBITERR,
                ENABLE);
            SDIO_DMACmd(ENABLE);
    ...
    if(DeviceMode == SD_INTERRUPT_MODE) {
        if(SDIO_GetITStatus(SDIO_IT_RXFIFOHF) != RESET) {
            ...
            if((TotalNumberOfBytes - NumberOfBytes) < SD_HALFFIFOBYTES) {
                restwords =
                    ((TotalNumberOfBytes - NumberOfBytes) % 4 == 0)
...
static SD_Error CmdResp6Error(u8 cmd, u16* prca)
{
    ...
    if(SD_ALLZERO ==
        (response_r1 & (SD_R6_GENERAL_UNKNOWN_ERROR | SD_R6_ILLEGAL_CMD |
